...
export default ({onPress, list, filter, filtering}) =>
    <aside >
        <input aria-labelledby="Restaurants Filter" className="filter-input" type="text" placeholder="Filter Restaurants" value={filtering} onChange={e => filter(e.target.value)}></input>
            {list.map(item => {
